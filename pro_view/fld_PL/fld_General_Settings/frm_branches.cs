...
namespace pro_view.fld_PL.fld_General_Settings
{
    public partial class frm_branches : Form
    {
        ...
        pro_view.fld_BL.cls_bl.stc_General_Settings.stc_branches module = new fld_BL.cls_bl.stc_General_Settings.stc_branches();
        ...
        void Form_Mode(string mode)
        {
            switch (mode)
            {
                ...
                case "New":
                    ...
                    com_company_id.Enabled = false;
                    txt_aname.Focus();
                    com_company_id.SelectedValue = frm_main.com_companies.SelectedValue.ToString();
                    break;
                #endregion
        ...
        void Bind()
        {
            ...
            module.company_id = com_company_id.SelectedValue.ToString();
        }
        void Refresh_Data()
        {
        ...
        {
            if (!frm_main.Perm("0101002"))
            {
                MessageBox.Show("ليس لديك تصريح لاضافة فرع جديد", Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            } 
        ...
        {
            if (!frm_main.Perm("0101003"))
            {
                MessageBox.Show("ليس لديك تصريح لتعديل بيانات فرع", Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            Tag = "Edit";
            Form_Mode("Edit");
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            if (com_company_id.SelectedValue == null)
            {
                MessageBox.Show("يجب اختيار الشركة التابع لها هذا الفرع ", "حفظ البيان", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                com_company_id.DroppedDown = true;
                return;
            }
            #endregion
        ...
        {
            if (!frm_main.Perm("0101004"))
            {
                MessageBox.Show("ليس لديك تصريح لحذف فرع", Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            if(txt_id.Text == frm_main.com_branches.SelectedValue.ToString())
            {
                MessageBox.Show("لا يمكن حذف الفرع الحالي المستخدم", "حذف", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return;
            }
