...
{
    public partial class frm_cout : Form
    {
        #region Declarations
        public pro_view.fld_PL.fld_Login.frm_main frm_main;
        pro_view.fld_BL.cls_bl.stc_Accounting.stc_cout module = new fld_BL.cls_bl.stc_Accounting.stc_cout();
        ...
        public frm_cout(fld_Login.frm_main frm_MAIN)
        {
            InitializeComponent();
        ...
        private void frm_Shown(object sender, EventArgs e)
        {
            ...
            if (txt_id.Text != "")
            {
                ...
                dt = ds.Tables["tbl_cout"].Clone();
                for (int i = 0; i < ds.Tables["tbl_cout"].Rows.Count; i++)
                {
                    dt.ImportRow(ds.Tables["tbl_cout"].Rows[i]);
                }
        ...
        void Form_Mode(string mode)
        {
            switch (mode)
            {
                ...
                case "Select":
                    ...
                    btn_Edit.Visible = (btn_Post.Text == "ترحيل") ? true : false;
                    btn_Save.Visible = false;
                    btn_Delete.Visible = (btn_Post.Text == "ترحيل") ? true : false;
        ...
        {
            if (!frm_main.Perm("0202002"))
            {
                MessageBox.Show("ليس لديك تصريح لاضافة سند صرف جديد", Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            Form_Mode("New");
        }
        ...
        {
            if (!frm_main.Perm("0202003"))
            {
                MessageBox.Show("ليس لديك تصريح لتعديل سند صرف", Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            Form_Mode("Edit");
        }
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataRow r in ds.Tables["tbl_fyears"].Rows)
            {
                if (r["id"].ToString() == com_fyear.SelectedValue.ToString())
                {
                    ...
                    {
                        MessageBox.Show("تاريخ السند خارج حدود السنة المالية " + fyearname, "سند صرف", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        txt_date.Focus();
                        return;
                    }
                }
            ...
            if (dgv.RowCount == 0)
            {
                MessageBox.Show("يلزم إدخال بيانات السند", "سند صرف", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                Add_Row();
                return;
            }
            ...
            for (int i = 0; i < dgv.RowCount; i++)
            {
                ...
                {
                    MessageBox.Show("لم يتم تحديد الحساب", "سند صرف", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        ...
        {
            if (!frm_main.Perm("0202004"))
            {
                MessageBox.Show("ليس لديك تصريح لحذف سند صرف", Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
        ...
        private void btn_Post_Click(object sender, EventArgs e)
        {
            if (Tag.ToString() == "Select")
            {
                ...
                {
                    if (!frm_main.Perm("0202005"))
                    {
                        MessageBox.Show("ليس لديك تصريح لترحيل سند صرف", Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return;
                    }
                ...
                {
                    if (!frm_main.Perm("0202006"))
                    {
                        MessageBox.Show("ليس لديك تصريح لإلغاء ترحيل سند صرف", Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return;
                    }
        ...
        private void com_paytype_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                dgv.CurrentCell = dgv.Rows[dgv.CurrentRow.Index].Cells[dgv.Columns["bank_name"].Index];
            }
        }
        #endregion
        ...
        {
            if (!frm_main.Perm("0202007"))
            {
                MessageBox.Show("ليس لديك تصريح لطباعة سند صرف", Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            #region Rep_Footer
            dt_Footer.Rows.Add("إجمالي المبلغ", "Total Amount", txt_Total.Text);
