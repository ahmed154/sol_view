...
namespace pro_view.fld_PL.fld_Accounting
{
    public partial class frm_is : Form
    {
        ...
        public bool Demo;
        fld_BL.cls_bl.stc_Accounting.IS module = new fld_BL.cls_bl.stc_Accounting.IS();
        ...
        void DecorateDGV()
        {
            foreach (DataGridViewRow row in dgv.Rows)
            {
                if (!(row.Cells["parent_debit"].Value is DBNull))
                {
                    ...
                    {
                        row.Cells["debit"].Style.Font = new Font(fb, FontStyle.Bold);
                ...
                if (!(row.Cells["parent_credit"].Value is DBNull))
                {
                    ...
                    {
                        row.Cells["credit"].Style.Font = new Font(fb, FontStyle.Bold);
        ...
        public void button_Display_Click(object sender, EventArgs e)
        {
            ...
            if(chk_OnlyLastLevel.Checked)
            {
                ...
                {
                    if (!(dr.Cells["debit"].Value is DBNull))
                    {
                        d += Convert.ToDecimal(dr.Cells["debit"].Value);
                    }
                    if (!(dr.Cells["credit"].Value is DBNull))
                    {
            ...
            if(d > c)
            {
                dt_result.Rows.Add(0, null, null, null, null, (d - c).ToString(), "خسائر مدورة", null, 1, null);
                c += (d - c);
            }
            else if(d < c)
            {
                dt_result.Rows.Add((c - d).ToString(), "أرباح مدورة", null, 1, null, 0, null, null, null, null);
                d += (c - d);
            }
        ...
        {
            if (!frm_main.Perm("0206002"))
            {
                MessageBox.Show("ليس لديك تصريح لطباعة قائمة الدخل", Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
        ...
        private void printDocument1_BeginPrint(object sender, PrintEventArgs e)
        {
            ...
            dt_PrintSettings.Rows.Add("IS", 90, 200, 105, 90, 200, 105);
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawString(((Page_RL) ? "قائمة الدخل" : "Income Sheet"), fh, Brushes.Black, rect, format);
        ...
        {
            if (!frm_main.Perm("0206003"))
            {
                MessageBox.Show("ليس لديك تصريح لتصدير قائمة الدخل للاكسيل", Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
